...
void wsize_change(FILE* fout, uint32_t node_id, Ptr<RdmaQueuePair> q){
	// uint32_t sid = ip_to_node_id(q->sip), did = ip_to_node_id(q->dip);
	// uint32_t total_bytes = q->m_size + ((q->m_size-1) / packet_payload_size + 1) * (CustomHeader::GetStaticWholeHeaderSize() - IntHeader::GetStaticSize()); // translate to the minimum bytes required (with header but no INT)
	// timestamp, node_id, sip, dip, sport, dport, sender_rate, window_size (HP)
	...
	// fprintf(fout, "%lu %u %08x %08x %u %u %lu\n", Simulator::Now().GetTimeStep(), node_id, q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->m_rate.GetBitRate());
	fflush(fout);
}

void sender_view_report(FILE* fout, uint32_t node_id, Ptr<RdmaQueuePair> q){
	// timestamp, node_id, sip, dip, sport, dport, hop_node_id, hop_node_qlen
	for (uint32_t i = 0; i < IntHeader::maxHop; i++){
		fprintf(fout, "%lu %u %08x %08x %u %u %u %lu\n",
			Simulator::Now().GetTimeStep(), node_id,
			q->sip.Get(), q->dip.Get(),
			q->sport, q->dport,
