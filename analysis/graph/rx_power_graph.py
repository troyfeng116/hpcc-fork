...
def process_rx_pkt_trace_file(file_name, node_num):
    # type: (str, int) -> Tuple[List[int], List[int], List[int]]
    times = []
    times_sent = []
    rx_pkt_szs = []
    ...
    with open(file_name, 'r') as file:
        ...
        # 4496 6 0b000001 0b000601 10000 79 1562 1234
        for line in file:
            toks = line.split()
            if len(toks) < 8:
            ...
            time_sent_ns = int(toks[7]) # time before sent in ns

            if node == node_num:
                times.append(time_ns)
                times_sent.append(time_sent_ns)
                rx_pkt_szs.append(rx_pkt_sz)

    print(len(times))
    return times, times_sent, rx_pkt_szs
...
if __name__ == '__main__':
    ...
    times, times_sent, rx_pkt_szs = process_rx_pkt_trace_file(file_name=trace_file, node_num=node_num)
    if not times:
        print("no receiver data for node {}. Are you sure this is a receiver?".format(node_num))
        exit(1)

    times_ms = [t / 1e6 for t in times]
    times_sent_ms = [t / 1e6 for t in times_sent]
    ...
    otts_ms = [
        t_arr - t_sent
        for (t_arr, t_sent) in zip(times_ms, times_sent_ms)
    ]
    tput_b_ms = [
        tbytes / t
    ...
    for i in range(1, len(times_ms)):
        ...
        ott_ms = otts_ms[i]
        tput = tput_b_ms[i-1]
        powers.append(tput / ott_ms)

    graph_title = get_graph_title(
        metric_name='Power (tput/delay, B/ms^2)',
        node_num=node_num,
        cc_algo=cc_algo,
        misrep=misrep,
    )
    out_png_name = get_out_png_filename(
        graph_metric='rx_power',
    ...
    plot_data_points(
        ...
        data_points=powers,
        xlabel='Time (ms)',
        ylabel='Power at receiver (B/ms^2)',
        title=graph_title,
